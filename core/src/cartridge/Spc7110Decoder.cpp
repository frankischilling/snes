...
uint8_t Spc7110Decoder::read() {
  ...
  uint8_t data = decomp_buffer[decomp_buffer_rdoffset++];
  decomp_buffer_rdoffset &= decomp_buffer_size - 1;
  decomp_buffer_length--;
  return data;
}

void Spc7110Decoder::write(uint8_t data) {
  decomp_buffer[decomp_buffer_wroffset++] = data;
  decomp_buffer_wroffset &= decomp_buffer_size - 1;
  decomp_buffer_length++;
}

uint8_t Spc7110Decoder::dataread() {
  const size_t size = rom_.size() > 0x500000 ? rom_.size() - 0x200000 :
                      rom_.size() > 0x100000 ? rom_.size() - 0x100000 : 0;
  if (!size) return 0;
  decomp_offset %= size;
  return rom_[0x100000 + decomp_offset++];
}

void Spc7110Decoder::init(unsigned mode, unsigned offset, unsigned index) {
  decomp_mode = mode;
...
void Spc7110Decoder::mode0(bool init) {
...
void Spc7110Decoder::mode1(bool init) {
...
void Spc7110Decoder::mode2(bool init) {
...
const uint8_t Spc7110Decoder::evolution_table[53][4] = {
...
const uint8_t Spc7110Decoder::mode2_context_table[32][2] = {
//{ next 0, next 1 },
...
uint8_t Spc7110Decoder::probability  (unsigned n) { return evolution_table[context[n].index][0]; }
uint8_t Spc7110Decoder::next_lps     (unsigned n) { return evolution_table[context[n].index][1]; }
uint8_t Spc7110Decoder::next_mps     (unsigned n) { return evolution_table[context[n].index][2]; }
uint8_t  Spc7110Decoder::toggle_invert(unsigned n) { return evolution_table[context[n].index][3]; }

unsigned Spc7110Decoder::morton_2x8(unsigned data) {
...
unsigned Spc7110Decoder::morton_4x8(unsigned data) {
