...
Internal CPU registers in `$00-$3F,$80-$BF:$4000-$43FF` take undriven bits from the shared CPU memory data latch. Reading those registers preserves that latch; other reads and all writes update it. The earlier implementation kept a separate latch for internal reads, which could return stale bits in status, controller and unused DMA registers. The regression cases cover both system-bank ranges, byte and word loads, and consecutive internal reads after ROM or WRAM access. The CPU suite now runs 1,171 checks; the new cases produced 216 failures before the fix and none afterward.
